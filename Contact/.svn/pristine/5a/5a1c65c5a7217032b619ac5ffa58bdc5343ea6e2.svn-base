/**
 * Created by hot on 2016-6-15.
 */
(function (angular) {
    'use strict';

    angular.module('App.contacts').factory('contactsListDataService',
        ['contactsListHttpService', '$q', 'commonContactlistDataService', 'contactDetailDataService',
            function (httpService, $q, commonContactlistDataService, contactDetailDataService) {
                var service = {};

                service.getBlobForContact = function (contacts) {
                    if (contacts.length) {
                        contacts.forEach(function (contact) {
                            (function (contact) {
                                if (contact.TitleDescriptor) {
                                    contact.TitleDescription = contact.TitleDescriptor.Description;
                                }
                                if (contact.AddressDescriptor) {
                                    contact.AddressDescrription = contact.AddressDescriptor.Description;
                                }
                                if (contact.CompanyBusinessUnitDescriptor) {
                                    contact.companyDescription = contact.CompanyBusinessUnitDescriptor.Code + ' ' + contact.CompanyBusinessUnitDescriptor.CompanyName + ' ';
                                }

                                contact.imageData = 'data:image/png;base64,';
                                contactDetailDataService.getBlobByContactId(contact.Id).then(function (data) {
                                    contact.contactBlob = {};
                                    if (data.length) {
                                        contact.imageData = 'data:image/png;base64,' + data[0].Blob.Content;
                                        contact.contactBlob = data[0].Blob;
                                    }
                                });
                            })(contact);
                        });
                    }
                };

                service.getContactList = function (options) {
                    var defer = $q.defer();
                    httpService.getContactList(options).then(function (response) {
                        try {
                            if (response.status === 200) {
                                commonContactlistDataService.setTotalRecords(response.data.FilterResult.RecordsFound);
                                service.getBlobForContact(response.data.Contacts);
                                defer.resolve(response.data);
                            } else {
                                defer.reject('connect fail');
                            }
                        } catch (error) {
                            defer.reject(false);
                        }

                    }, function (error) {
                        defer.reject(error);
                    });
                    return defer.promise;
                };

                return service;
            }]);

})(angular);