...
(function (angular) {
	...
	angular.module('platform').factory('platformsDesService', platformsDesService);

	function platformsDesService() {
		var service = {};
		service.des = function (key, message, encrypt, mode, iv) {//jshint ignore:line
			...
			var keys = service.desCreateKeys(key);
			var m = 0,
			...
			while (m < len) {
				...
				encrypt ? (chunk += 16) : (chunk += 8);//jshint ignore:line
		...
		service.desCreateKeys = function (key) {
			...
			var lefttemp, righttemp, m = 0, left, right, i,
				n = 0,
				temp;
		...
		service.stringToHex = function (s) {
			var r = '';
		...
		service.HexTostring = function (s) {
			var r = '';
		...
		//JS DES加密函数
		service.jsencrypt = function (key, message) {
			var ciphertext = service.stringToHex(service.des(key, message, 1, 0));
			return ciphertext;
		};

		//JS DES解密函数
		service.jsdecrypt = function (key, message) {
			var plaintext = service.des(key, service.HexTostring(message), 0, 0);
			return plaintext;
		};
		...
		service.encrypt = function (str) {
			try {
				return service.jsencrypt('sjdfown#$*79adf2!09802jhod2jklsaf(*(w3%$%^#$#', str);
				//return str;
		...
		service.decrypt = function (str) {
			try {
				if (parseInt('0x' + str).toString().length >= 16) {
					return service.jsdecrypt('sjdfown#$*79adf2!09802jhod2jklsaf(*(w3%$%^#$#', str);
				}
				else {
					return str;
				}
		...
		return service;
		//how to use
		// ControlTower.lib.des.jsencrypt("123" , 'Myth');
		//ControlTower.lib.des.jsdecrypt("123" , 'BF4134D52DBA0FB2');
	}
})(angular);
